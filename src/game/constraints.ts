import type { ActiveConstraint, ConstraintDefinition, ConstraintKind } from './types';
...
export function createNextConstraint(
  ...
  const activeKinds = new Set(activeConstraints.map((constraint) => constraint.kind));
  const candidateKinds = CONSTRAINT_KINDS.filter(
    (kind) => !activeKinds.has(kind) && canAddConstraintKind(kind, activeConstraints, word),
  );
...
export function describeConstraint(activeConstraint: ActiveConstraint): ConstraintDefinition {
  switch (activeConstraint.kind) {
    ...
    case 'no-dakuten':
      return {
        ...
        validate: (word) => !/[\u3099\u309a]/u.test(normalizeWordInput(word).normalize('NFD')),
      };
    ...
    case 'no-small-kana':
      return {
        ...activeConstraint,
        title: '小さいかな禁止',
        description: 'ゃゅょっ等を含めない',
        failureMessage: '小さいかなは使えません。',
        validate: (word) =>
          !/[ぁぃぅぇぉゃゅょっゎァィゥェォャュョッヮ]/u.test(normalizeWordInput(word)),
      };
    ...
    case 'require-specific-kana': {
      const requiredKana = activeConstraint.requiredKana ?? REQUIRED_KANA_OPTIONS[0];
...
function createConstraint(kind: ConstraintKind, random: () => number): ActiveConstraint {
  switch (kind) {
    case 'min-length': {
